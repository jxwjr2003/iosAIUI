...
const mock_server_manager_1 = require("./mock-server/mock-server-manager");
...
const multi_path_config_manager_1 = require("./storage/multi-path-config-manager");
class LocalProxyApp {
    constructor() {
        this.mainWindow = null;
        this.mockServerEngine = new mock_server_engine_1.MockServerEngine();
        this.configManager = new config_manager_1.ConfigManager();
        this.multiPathConfigManager = new multi_path_config_manager_1.MultiPathConfigManager(this.configManager);
        this.mockServerManager = new mock_server_manager_1.MockServerManager(this.configManager);
    ...
    setupIpcHandlers() {
        ...
        // 新增批量启动接口
        electron_1.ipcMain.handle('mock-server:start-all', async () => {
            try {
                const result = await this.mockServerManager.startAll();
                // 记录批量启动结果
                if (result.success) {
                    this.logManager.addLog('mock-server', 'info', `Started ${result.started.length} mock servers, ${result.failed.length} failed`);
                }
                else {
                    this.logManager.addLog('mock-server', 'error', `Batch start failed: ${result.started.length} started, ${result.failed.length} failed`);
                }
                // 记录端口冲突
                ...
                result.failed.forEach(failure => {
                    this.logManager.addLog('mock-server', 'error', `Failed to start server ${failure.id}: ${failure.error}`);
                });
                return result;
            }
            catch (error) {
        ...
        // 获取所有服务器状态
        electron_1.ipcMain.handle('mock-server:get-statuses', async () => {
            try {
        ...
        // 停止单个服务器
        electron_1.ipcMain.handle('mock-server:stop-server', async (event, configId) => {
            try {
        ...
        // 多路径配置管理器 IPC 处理程序
        electron_1.ipcMain.handle('storage:get-locations', async () => {
            try {
        ...
        electron_1.ipcMain.handle('storage:switch', async (event, providerId) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:add-custom', async (event, customPath, name) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:load-from-file', async (event, filePath) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:watch-file', async (event, filePath) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:stop-watch-file', async (event, filePath) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:export-config', async (event, config, options) => {
            try {
        ...
        electron_1.ipcMain.handle('storage:import-config', async (event, configData) => {
            try {
                // 处理从配置数据对象导入
                const tempFilePath = path.join(electron_1.app.getPath('temp'), `import-${Date.now()}.json`);
                const fs = require('fs');
        ...
        electron_1.ipcMain.handle('storage:generate-export-filename', async (event, baseName) => {
            try {
        ...
        // 添加UI所需的存储管理IPC处理程序
        // 获取当前存储位置
        electron_1.ipcMain.handle('storage:get-current-location', async () => {
            try {
        ...
        // 设置存储位置
        electron_1.ipcMain.handle('storage:set-location', async (event, path) => {
            try {
                let result;
                if (path === 'default') {
        ...
        // 迁移数据
        electron_1.ipcMain.handle('storage:migrate-data', async (event, newPath) => {
            try {
                // 先添加新的存储位置
        ...
        // 浏览路径
        electron_1.ipcMain.handle('storage:browse-path', async () => {
            try {
        ...
        // 保存导出
        electron_1.ipcMain.handle('storage:save-export', async (event, options) => {
            try {
        ...
        // 加载动态文件
        electron_1.ipcMain.handle('storage:load-dynamic-file', async (event, filePath) => {
            try {
                const config = await this.multiPathConfigManager.loadFromFile(filePath);
        ...
        electron_1.ipcMain.handle('dialog:save-file', async (event, options) => {
            try {
     ...
     * 设置多路径配置管理器事件监听
     */
    setupMultiPathConfigManagerEvents() {
        // 监听存储切换事件
     ...
     * 设置代理服务器引擎事件监听
     */
    setupProxyServerEngineEvents() {
        // 监听代理服务器请求接收事件
        this.proxyServerEngine.on('requestReceived', (proxyRequest) => {
            // 记录代理服务器请求详细信息
            this.logManager.addLog('proxy-server', 'info', `Proxy Request: ${proxyRequest.method} ${proxyRequest.url}`, {
                type: 'proxy-request',
                id: proxyRequest.id,
        ...
        // 监听代理服务器响应发送事件
        this.proxyServerEngine.on('responseSent', (data) => {
            const { request, response } = data;
            // 记录代理服务器响应详细信息
            this.logManager.addLog('proxy-server', 'info', `Proxy Response: ${response.statusCode} for ${request.method} ${request.url}`, {
                type: 'proxy-response',
                id: response.id,
                timestamp: response.timestamp,
                statusCode: response.statusCode,
        ...
        // 监听代理服务器启动事件
        this.proxyServerEngine.on('serverStarted', (config) => {
            this.logManager.addLog('proxy-server', 'info', `Proxy server started on port ${config.port}`);
        });
        // 监听代理服务器停止事件
